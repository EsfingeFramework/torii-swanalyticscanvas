...
const ToDoComp = (props) => {
    ...
    const updateSubTask = (index,state) =>{
        ...
        const subTask={"id":id,"category":category,"description":description,"color":color,"status":tempStatus,"members":members,"complete":complete,"connected":connected,"connFlag":tempConnFlag,"connPrev":[], "creator":creator, "pId":pId,"tId":tId}
    ...
    return (
        <div style={{marginTop:"5%"}}>
           {connected.map((get,index) => {
                return (
                    <Text align="left" weight={700} style={{marginBottom:"3%",marginRight:"4%",marginLeft:"4%"}}>
                        <Checkbox onClick={() => {updateSubTask(index,1)}} checked={connFlag[index] === false ? false : true} size="md"/>
                        - {get}
                    </Text>
                )
            })}
