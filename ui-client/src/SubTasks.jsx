...
import {Modal, Button, Group, Divider, Switch, Avatar, Text, Title, MultiSelect} from '@mantine/core';
import { IconButton } from "@mui/material";
import AccountCircleIcon from '@mui/icons-material/AccountCircle';
import Slider from '@mui/material/Slider';
import axios from "axios";
import {useLocation} from "react-router-dom";
import {Center, ColorInput, MantineProvider, Space, Textarea, TextInput} from "@mantine/core";
...
const SubTasks = (props) => {
    ...
    const addSubTask = () =>{
        const subTask={"category":category,"description":description,"color":color,"status": "pending","members":members,"complete":parseInt(complete),"date":date, "deadline":deadline, "connected":connected, "connFlag":[], "connPrev":[], "creator":creator, "pId":pId, "tId":tId}
        console.log(JSON.stringify(subTask))
        fetch("http://localhost:8080/api/subTask",{
            method: "POST",
            headers:{"Content-type":"application/json"},
            body:JSON.stringify(subTask)}).then(async () => {
    ...
    const addIssue = () =>{
        const subTask={"category":"issue","description":taskData.description,"color":color,"status": "pending","members":members,"complete":parseInt(complete),"connected":connected, "connFlag":[], "connPrev":[], "creator":creator, "pId":pId, "tId":tId}
        console.log(JSON.stringify(subTask))
        fetch("http://localhost:8080/api/subTask",{
            method: "POST",
            headers:{"Content-type":"application/json"},
    ...
    const updateTaskIssueStatus = () =>{
        ...
        const mainTask={"id":tId, "description":taskData.description,"date":taskData.date,"deadline":taskData.deadline,"members":taskData.members,"complete":taskData.complete,"connected":taskData.connected,  "pId":pId,"generated":false,"issue":true,"measure":true,"tool":true,"high":true,"insight":true,"goal":true,"decision":true, "hasIssue":true}
        console.log(JSON.stringify(mainTask))
        fetch("http://localhost:8080/api/mainTask",{
            method: "PUT",
            headers:{"Content-type":"application/json"},
            body:JSON.stringify(mainTask)}).then(()=> {
    ...
    const updateTask = (flag,path) =>{
        ...
        const mainTask={"id":tId, "description":taskData.description,"date":taskData.date,"deadline":taskData.deadline,"members":taskData.members,"complete":taskData.complete,"connected":taskData.connected,  "pId":pId,"generated":false,"issue":tempIssue,"measure":tempMeasure,"tool":tempTool,"high":tempHigh,"insight":tempInsight,"goal":tempGoal,"decision":tempDecision, "hasIssue":hasIssue}
        console.log(JSON.stringify(mainTask))
        fetch("http://localhost:8080/api/mainTask",{
            method: "PUT",
            headers:{"Content-type":"application/json"},
        ...
        <Box  sx={{ flexGrow: 1 }}>
            ...
            <Grid container textAlign="center">
                <Grid item md={2} xs={12}>
                    <h2><BugReportIcon/> Key Issue</h2>
                    <Divider my="sm" style={{marginLeft:"4%", marginRight:"4%"}}/>
                        <div>{gets.map((get,index) => {
                ...
                <Grid item md={2} xs={12}>
                    <h2><InsightsIcon /> Measurements</h2>
                    <Divider my="sm" style={{marginLeft:"4%", marginRight:"4%"}}/>
                    <div>{gets.map((get,index) => {
                ...
                <Grid item md={2} xs={12} >
                    <h2><ConstructionIcon/> Methods & Tools</h2>
                    <Divider my="sm" style={{marginLeft:"4%", marginRight:"4%"}}/>
                    <div>
                            {gets.map((get,index) => {
                ...
                <Grid item md={2} xs={12} >
                    <h2><TipsAndUpdatesIcon/> Insights</h2>
                    <Divider my="sm" style={{marginLeft:"4%", marginRight:"4%"}}/>
                    <div>
                            {gets.map((get,index) => {
                ...
                >
                    ...
                    <br/>
                    <Center>
                        <Button variant="outline" text color="green" onClick={() =>{updateTask(category, 1)}}>
                            Add
                        </Button>
                    </Center>
                </Modal>
