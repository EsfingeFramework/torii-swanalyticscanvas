...
const SubTaskComp = (props) => {
    ...
    const addSubTask = () =>{
        const subTask={"category":category,"description":description,"color":color,"status": "pending","members":members,"complete":parseInt(complete),"connected":[], "connFlag":[], "connPrev":[parentDescription], "creator":creator, "pId":pId, "tId":tId}
    ...
    const updateSubTask = () =>{
        ...
        const subTask={"id":id,"category":category,"description":description,"color":color,"status":status,"members":members,"complete":complete,"connected":connected,"connFlag":connFlag,"connPrev":connPrev,"creator":creator, "pId":pId,"tId":tId}
        console.log(JSON.stringify(subTask))
        fetch("http://localhost:8080/api/subTask/",{
            method: "PUT",
            headers:{"Content-type":"application/json"},
    ...
    const updateSubTask2 = (e) =>{
        ...
        const subTask={"id":id,"category":category,"description":parentDescription,"color":color,"status":status,"members":members,"complete":complete,"connected":tempConn,"connFlag":tempConnFlag,"connPrev":connPrev,"creator":creator, "pId":pId,"tId":tId}
        console.log(JSON.stringify(subTask))
        fetch("http://localhost:8080/api/subTask/",{
            method: "PUT",
            headers:{"Content-type":"application/json"},
    ...
    const updateStatus = (reload) =>{
        ...
        const subTask={"id":id,"category":category,"description":parentDescription,"color":color,"status":tempStatus,"members":members,"complete":complete,"connected":connected,"connFlag":connFlag,"connPrev":connPrev,"creator":creator, "pId":pId,"tId":tId}
        console.log(JSON.stringify(subTask))
        fetch("http://localhost:8080/api/subTask/",{
            method: "PUT",
            headers:{"Content-type":"application/json"},
    ...
    return(
      <div>
          <Card
              ...
              component="a"
              target="_blank"
              style={{marginLeft:"4%", marginRight:"4%",cursor: "pointer",marginBottom:"2%"}}
          >
          <Card.Section onClick={handleOpen}>
              <Group position="apart" style={{marginBottom:"3%",marginLeft:"4%", marginRight:"4%", marginTop:"3%"}}>
                  <Avatar alt={creator} color="cyan" size="sm">{creator.slice(0, 2).toUpperCase()}</Avatar>
                  <Badge color="teal" size="xs" radius="sm" >{category}</Badge>
                      <Text  align="center"  size="xs" style={{color: getStatusColor()}}>
                          {status}
                      </Text>
              </Group>

              <Text style={{marginBottom:"3%",marginLeft:"4%", marginRight:"4%",}} align="center" weight={500} size="lg">
                  {description}
              </Text>
              <Progress style={{marginBottom:"3%",marginLeft:"4%", marginRight:"4%",}} value={100} color={color} size="sm" radius="xl" />
          </Card.Section>
              <Card.Section>
              <div hidden={!openedDetails}>
                  <Badge size="lg" radius="sm" style={{marginTop:"4%"}}>Tasks</Badge>
                  <ToDoComp
                      id = {id}
                      category = {category}
                  ...
                  <Group position="center" >
                      <IconButton onClick={()=>{openModal(description)}} color="primary" aria-label="upload picture" component="span" size="small">
                          <AddIcon fontSize="medium"/>
                      </IconButton>
                      <Space w="xl" />
                      <IconButton hidden={status !== "planned" ? false : true} onClick={()=>{openNextModal(category,description)}} color="success" aria-label="upload picture" component="span" size="small" >
              ...
              <Card.Section>
                  <Center>
                      <IconButton style={{color:"white"}} onClick={() => setOpenedDetails((o) => !o)}>
            ...
            <Modal
              ...
              >
                <br/>
                {category !== "issue" ? <div><Text size="xl">Created because of: <Badge color="teal" radius="sm" >{prevCat()}</Badge></Text><Center><Text size="xl">{connPrev[0]}</Text></Center></div>: <div></div>}
                <br/>
                <Textarea
                ...
                />*/}
                {(category === "issue" || category ==="measurements") ?<div></div> :
                    <Center>
                        {(category === "goals" || category === "decisions") ?
                            <Button variant="outline" text color="primary" onClick={() =>{updateStatus(true)}}>
                                Completed
                            </Button> :
                            <Button variant="outline" text color="primary" onClick={() =>{updateStatus(true)}}>
                                no {nextCat()} found
          ...
          <Modal
              ...
              title="New"
          >
              {(category === "goals" || category === "decisions") ? <Select
                  label="Select"
